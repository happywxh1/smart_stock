...
public class CSwiftV {
    ...
    public init(with string: String, separator: String = ",", headers: [String]? = nil) {
        ...
        self.columnCount = self.rows.count
        
        for row in rows {
            print(row.enumerated())
        }
        /*
        rows.map { field -> [String: String] in
            //var row = [String: [String : String]]()
            // Only store value which are not empty
            let key = field.enumerated()
            print(key)
    ...
    private static func split(_ separator: String, string: String) -> [String] {
        func oddNumberOfQuotes(_ string: String) -> Bool {
            let separateCount = string.components(separatedBy: "\"").count;
            return separateCount % 2 == 0 && separateCount > 1
        }
